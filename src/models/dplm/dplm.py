...
class DiffusionProteinLanguageModel(nn.Module):
    _default_cfg = DPLMConfig()  # 默认配置

    def __init__(self, cfg, net=None):
        super().__init__()
        self._update_cfg(cfg)   # 更新配置（合并用户配置和默认配置）

        # 初始化网络：如果没传入 net，就根据 cfg 创建
    ...
    def from_pretrained(
        ...
        if not from_huggingface:
            ...
            # 修改逻辑：从本地加载 HuggingFace 模型
            local_dir = "airkingbd/dplm_650m"
            ...
            with open(config_path, "r") as f:
                config = json.load(f)
            dplm_type = config.get("dplm_type")
