...
@register_model("dmlm_esm")
class EsmForDMLM(EsmForMaskedLM):
    def __init__(self, config, dropout=0.1):
        print("[Info] Using CrystaTokenizerWrapper for crystal generation.")
        tokenizer = CrystaTokenizerWrapper()
        config.hidden_dropout_prob = dropout
        if config.vocab_size != len(tokenizer):
            print(
                f"[Warning] Vocab size mismatch: model={config.vocab_size}, tokenizer={len(tokenizer)}. Adjusting config.vocab_size.")
            config.vocab_size = len(tokenizer)

        print("[Info] Function dmlm_modeling_sem.py EsmForDMLM.__init__ : EsmPreTrainedModel.__init__.")
        EsmPreTrainedModel.__init__(self, config)
        ...
        print("[Info] Function dmlm_modeling_sem.py EsmForDMLM.__init__ Done.")
    ...
    def generate(
        ...
        """
        Args:
            batch:  提供生成所需的条件信息或起始状态。
            tokenizer: 将生成的token ID转换回人类可读的序列。
            max_iter: 控制去噪/生成过程的步数，影响生成质量
            temperature: 控制采样的随机性/多样性，影响生成结果的多样性。
            partial_masks: 条件生成：控制哪些位置已知（不改变），哪些位置需要生成。
            sampling_strategy: 决定如何从模型的概率分布中选择token
        ...
        # 0) encoding
        encoder_out = self.forward_encoder(batch)  # 通常返回空字典
