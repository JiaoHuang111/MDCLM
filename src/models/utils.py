...
def get_net(cfg):
    log.info(f'Function utils.get_net Start!')
    if cfg.net.arch_type == "esm":
        log.info(f'Function utils.get_net step1: cfg.net.arch_type == "esm"!')
    ...
    elif cfg.net.arch_type == "esm_crystal":
        log.info(f'Function utils.get_net step1: cfg.net.arch_type == "esm_crystal"!')
        from byprot.models.dplm.modules.dmlm_modeling_esm import EsmForDMLM

        config = AutoConfig.from_pretrained(f"{cfg.net.name}")
        net = EsmForDMLM(config, dropout=cfg.net.dropout)
    # TODO: dplm will support more architectures, such as Llama
    else:
        log.info(f'[Error] Function utils.get_net step1: cfg.net.arch_type None!')
    ...
    log.info(f'Function utils.get_net step 2: training!')
    if cfg.net.pretrain:
        log.info(f'Function utils.get_net step 2 stage 1: pretrain a (MLM)!')
        pretrained_model_name_or_path = cfg.net.pretrained_model_name_or_path
        is_local = os.path.exists(pretrained_model_name_or_path)
        if is_local:
            log.info(f'Function utils.get_net step 2 stage 2: load your pretrained model from local!')
            ...
            log.info(f'Function utils.get_net step 2 stage 2: load your pretrained model from huggingface!')
    ...
    log.info(f'Function utils.get_net step 3: activate lora training!')
    ...
    log.info(f'Function utils.get_net Done!')
    return net
