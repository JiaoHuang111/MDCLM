...
def common_pipeline(config, training=False):
    import pprint
    # 保存完整配置
    OmegaConf.save(config, "debug_config.yaml")
    log.info("Full config saved to debug_config.yaml")
    ...
    # 根据配置实例化模型任务模块（LightningModule），封装了 forward、loss 和优化器

    log.info(f"Function common_pipeline step 3:")
    log.info(f"Init lightning loggers")
    ...
    if "logger" in config:
        for _, lg_conf in config.logger.items():
            if "_target_" in lg_conf:
                ...
                logger.append(lg)
                # 实例化日志系统，比如 TensorBoardLogger 或 WandbLogger
                ...
                if isinstance(lg, TensorBoardLogger):
                    ...
                    # 避免 tensorboard 首次保存超参数时报错，提前建文件

    # Init lightning callbacks
    log.info(f"Function common_pipeline step 4:")
    log.info(f"Init lightning callbacks")
    ...
    if "callbacks" in config:
        for cb_name, cb_conf in config.callbacks.items():
            ...
            if "_target_" in cb_conf:
                ...
                # 实例化回调函数，比如学习率调度、early stopping
        ...
        if config.trainer.get("enable_progress_bar", False):
            ...
            # 使用自定义进度条（替代 Lightning 默认进度条）
    log.info(f"Function common_pipeline Done!")
